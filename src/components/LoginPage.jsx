...
export default function LoginPage() {
  ...
  async function handleEmailLogin() {
    if (!email || !password) { setError(t('login_error_fields')); return; }
    setError('');
  ...
  async function handlePasswordSetup() {
    if (newPw !== confirmPw) { setPwError(t('pw_error_match')); return; }
    if (newPw.length < 8)    { setPwError(t('pw_error_length')); return; }
    setPwError('');
    setPwLoading(true);
    try {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', background: C.bg }}>
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: '44px 44px 40px', width: 420, boxShadow: '0 8px 32px rgba(0,0,0,0.08)' }}>
          ...
          <div style={{ fontSize: 18, fontWeight: 700, color: C.text, marginBottom: 6, letterSpacing: '-0.01em' }}>{t('pw_setup_title')}</div>
          <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.6, marginBottom: 28 }}>{t('pw_setup_sub')}</div>

          <div style={{ marginBottom: 16 }}>
            <label style={labelStyle}>{t('pw_new_label')}</label>
            <input type="password" value={newPw} onChange={e => setNewPw(e.target.value)}
              placeholder={t('pw_new_ph')} style={inputStyle} />
          </div>
          <div style={{ marginBottom: 24 }}>
            <label style={labelStyle}>{t('pw_confirm_label')}</label>
            <input type="password" value={confirmPw} onChange={e => setConfirmPw(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && handlePasswordSetup()}
              placeholder={t('pw_confirm_ph')} style={inputStyle} />
          </div>
          ...
          <button onClick={handlePasswordSetup} disabled={pwLoading || !newPw || !confirmPw}
            ...
            {pwLoading ? t('pw_saving') : t('pw_btn')}
          </button>
        </div>
      </div>
    );
    ...
    <div className="login-split" style={{ display: 'flex', height: '100vh' }}>
      ...
      <div className="login-brand" style={{ width: 420, background: '#0C0D10', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '52px 48px 44px', flexShrink: 0 }}>
        <div>
          ...
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.28)', letterSpacing: '0.22em', textTransform: 'uppercase', marginTop: 6 }}>{t('nav_subtitle')}</div>
        </div>
        <div>
          <div style={{ fontSize: 30, fontWeight: 700, color: '#F5F4F2', lineHeight: 1.3, marginBottom: 20, letterSpacing: '-0.02em' }}>
            {t('brand_tagline').split('\n').map((line, i) => <React.Fragment key={i}>{line}{i === 0 ? <br /> : null}</React.Fragment>)}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {[
              { icon: '◆', textKey: 'feature_sync' },
              { icon: '◎', textKey: 'feature_email' },
              { icon: '⬡', textKey: 'feature_views' },
            ].map(f => (
              <div key={f.textKey} style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                <span style={{ color: C.accent, fontSize: 15, flexShrink: 0 }}>{f.icon}</span>
                <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.45)' }}>{t(f.textKey)}</span>
              </div>
            ))}
          </div>
        </div>
        <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)' }}>© {new Date().getFullYear()} LEAGL — {t('copyright')}</div>
      </div>

      {/* Right — form panel */}
      <div className="login-form" style={{ flex: 1, background: C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 48, position: 'relative' }}>
        {/* Language switcher top-right */}
        <div style={{ position: 'absolute', top: 20, right: 24, display: 'flex', gap: 4 }}>
          {['nl', 'fr', 'en'].map(l => (
            <button key={l} onClick={() => setLang(l)}
              style={{ padding: '4px 10px', fontSize: 12, fontWeight: lang === l ? 700 : 400, background: lang === l ? C.border : 'transparent', color: lang === l ? C.text : C.muted, border: `1px solid ${lang === l ? C.border : 'transparent'}`, borderRadius: 6, cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
              {l}
            </button>
          ))}
        </div>
        <div style={{ width: '100%', maxWidth: 380 }}>
          ...
          <div style={{ marginBottom: 36 }}>
            <div style={{ fontSize: 24, fontWeight: 700, color: C.text, marginBottom: 8, letterSpacing: '-0.02em' }}>{t('login_welcome')}</div>
            <div style={{ fontSize: 14, color: C.muted }}>{t('login_subtitle')}</div>
          </div>
          ...
          {ssoLoading ? (
            <div style={{ textAlign: 'center', padding: '40px 0', color: C.muted }}>
              ...
              <div style={{ fontSize: 14 }}>{t('login_sso_loading')}</div>
            </div>
            ...
            <>
              <div style={{ marginBottom: 16 }}>
                <label style={labelStyle}>{t('login_email_label')}</label>
                <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                  placeholder="naam@bedrijf.be" style={inputStyle} />
              </div>
              <div style={{ marginBottom: 24 }}>
                <label style={labelStyle}>{t('login_pw_label')}</label>
              ...
              <button onClick={handleEmailLogin} disabled={loginLoading}
                ...
                {loginLoading ? t('login_loading_btn') : t('login_btn')}
              </button>
            ...
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 24 }}>
                ...
                <span style={{ fontSize: 12, color: C.muted, fontWeight: 500 }}>{t('login_or_email')}</span>
                <div style={{ flex: 1, height: 1, background: C.border }} />
              </div>
